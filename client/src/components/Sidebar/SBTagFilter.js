  ...
  return (
    <SideBarSection>
      ...
      {props.tags.sort().map(tag => (
        <li className='rounded-lg text-sm mb-2 py-2px list-reset overflow-hidden cursor-pointer tagLinkInactive' key={tag} onClick={(event) => props.toggleSelectTag(event, tag)}>
          {tag}
        </li>
      ))}
    </SideBarSection>
